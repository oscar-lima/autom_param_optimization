...
# Tested On
-18.04 melodic,kinetic ros1 , did not work out of the box,may require some work,have not tried deeply

# Usage
(for testing the working with their data and on ros1 (melodic/kinetic))
...
---To test/train with your own data

1)First generate your bag file-
 Steps-

1. Get AMCL working with your world model,and map
(reference- https://emanual.robotis.com/docs/en/platform/turtlebot3/navigation/ ) 
2.Generate 3 additional topics needed-
...
3.Record all these topics as a bag file in addition to the ones published by the amcl setup or you can even specifically record only the required topics as below-
...
(Note- amcl_pose is included because it is needed later to compare between amcl_pose(using these default parameters) and the amcl_pose computed by SMAC tuning process)

Note- For recording own bag file - make sure to record a larger size of file at least 15-20 mb and also a good duration minimum of 3-5 min.

2)Filter the recorded bag file

The bag file as input to the SMAC algorithm should not contain map-to-odom transform and /amcl_pose as recorded above,because these will be  published by the SMAC algorithm itself.So these topics need to be filtered out of the bag file-
...
If needed you can include additional topics in the above command,but these are the minimum required topics.Note-In the above command,make sure that "m.transforms[0].child_frame_id != "odom"" comes after "/tf" and not "tf_static".Reference-(https://answers.ros.org/question/56935/how-to-remove-a-tf-from-a-ros-bag/)

4)Visualize the tf-tree of the filtered bag file.Tf-tree for reference- (Attach)

5)-Change the map path in amcl_dependencies.launch, Change the map path in amcl_instance_server.launch,Change the bag_file path in amcl_run_bag.launch.Check the other launch files,if any other paths are different for your setup.In the amcl_run_bag.launch ,you can change the start and duration time if neeeded.
...
6)Once the above steps are completed for your custom data,as done for their data previously,run the amcl server and client commands and visualize the localization in rviz.
You can also get a score value for your data using the command- rosrun mbot_autom_param_tuning_tools smac_wrapper_node amcl no_instance ,lower the score,better is your localization.

7)This step is optional- You can also change the friction parameters for your turtlebot(if using) by changing the mu1,mu2 parameters in .xacro file of the particular turtlebot3 model (example file- https://github.com/ROBOTIS-GIT/turtlebot3/blob/master/turtlebot3_description/urdf/turtlebot3_burger.gazebo.xacro).For more details refer - http://gazebosim.org/tutorials?tut=friction

8)This step is optional- Compute the error between the SMAC estimated amcl_pose and ground_truth.Reference scripts - (Attach 3)
...
Usage- 
Run these scripts(script 1,2,3) in parallel(in different terminals) to the SMAC algorithm,so that these scripts can subscribe to the topics as published by the SMAC.Once the text files are generated ,use these to visualize the graph and compute the MSE.

To compare this SMAC MSE to the amcl_pose recorded in the bag file (topic /amcl_pose),play the recorded bag file (not the filtered one) and run the same scripts(1,2 and 3) again to get the text files and compute the MSE.(in other words repeat the same steps as done for computing the MSE for SMAC and for graph visualization of SMAC.)

Now the MSE of /amcl_pose(the pose with default amcl parameters) and the MSE of SMAC estimated pose can be compared,to check how much the error has reduced after tuning.

9)To train or tune the SMAC algorithm to your custom data-
(This step is needed if either the MSE is too high or localization needs further improvement)
Example of the Command used to tune- rosrun mbot_autom_param_tuning_tools smac --use-instances false --numberOfRunsLimit 100 --pcs-file example_scenarios/amcl/params.pcs --algo "rosrun mbot_autom_param_tuning_tools smac_wrapper_node amcl" --run-objective QUALITY
...
While the above command is running,you can visualize the localization in rviz,(as show in the youtube video).You can change the default_amcl_parameters to start the training with ,or the duration of the bag file or the number of iterations,according to the custom data.
...
FOR ROS2
You can use rosbridge,and directly record a bag file in ros1 or setup the world and map file in ros1.
(Sources for ros-bridge-https://github.com/ros2/ros1_bridge/blob/foxy/README.md )
...
Common ERRORS-
1)can transform error.. look into future...- if it publishes constantly- then make sure your tf tree and transforms are correct. It might be due to some timesource issue.The algo will still seem to work ,but you will not know which are the best parameters.

2)process has died amcl- mainly due to short bag file.
