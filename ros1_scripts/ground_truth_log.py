...
class MinimalSubscriber():
    ...
    def __init__(self):
        #the path to the file where the ground_truth_pose values will be stored,when this script is running.
        self.f2=open('./gt.txt','a+')
...
def main(args=None):
    rospy.init_node('log_gt_pose', anonymous=True)
    obj=MinimalSubscriber()
    sub=rospy.Subscriber('/vrpn_client_node/base_link_gt/pose',PoseStamped,obj.callback) #topic on which the ground_truth is published.
    rospy.spin()
